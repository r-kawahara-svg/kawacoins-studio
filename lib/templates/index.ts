...
制度変更・改正がテーマの場合は、本文中で「現行」と「改正後（予定）」を必ず対比して説明する。

[TABLE:制度概要]
※制度変更がテーマなら、この表は「項目／現行／改正後（予定）」の3列構成にし、現行と改正後を一目で比較できる形にする。未確定の数値には「（予定）」を付ける。
...
## 考察
[INSIGHT]
※ここに、表面的な制度説明では終わらない筆者独自の考察を2〜3文で書く。
「この改正は〇〇なキャリア・状況の人に特に有利／不利」「実務の経験から見ると〜という戦略が有効」など、
読者が他サイトでは得られない実践的な視点を一人称で述べる。「コンサル」という単語自体は本文に書かない。
