...
${currentYear - 1}年以前を「最新」「今年」と書かない。古い年号は${currentYear}年に更新する。

【制度・改正の確実性ルール（本文とFAQで統一）】
- 数値や制度内容は「施行済み（確定）」「審議中」「予定」を区別する。
- 未確定の内容を断定しない。「〜される予定」「現在審議中」「〜の見込み」と書く。
- 本文・FAQ・会話吹き出しでトーンを食い違わせない。
...
- 記事内で触れたデメリット・不安（手数料・難しさ等）を、紹介サービスがどう解消するかを1文で結びつける。
...
- 筆者考察: 骨格に [INSIGHT] がある場合、[INSIGHT]考察本文[/INSIGHT] で「誰にどう効くか」の実践的な独自考察を2〜3文。一人称で書き、「コンサル」という単語は使わない。
- 箇条書き: 3つ以上は必ずMarkdownリスト

${GUARDRAILS}`;

  const userPrompt = skeleton
    ? `以下の「現在の本文」を読み、方針に従って改善した記事を3000文字程度で完成させてください。
良い部分は活かし、不自然・冗長・古い箇所を直す形で書き直します（ゼロから別物にしない）。
...
---
[TABLE:XXX] / [STEPS:XXX] / [CHART:XXX] / [FAQ] / [AFFILIATE:XXX] / [EXPERIENCE:XXX] / [INSIGHT]...[/INSIGHT] はプレースホルダ。現在の本文にあるものは残し、無ければ骨格に従って配置する（後処理で置換）。
