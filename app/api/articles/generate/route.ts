...
// 筆者プロフィール（独自考察 [INSIGHT] の根拠に使う）
const AUTHOR_PROFILE = `個人投資家。株式投資の実体験に加え、ビジネスの実務経験を持つ。
制度や数字を「読者個人のキャリア・ライフプランにどう効くか」という実践的な視点で読み解くのが強み。`;
...
- アフィリエイトリンクの挿入箇所には [AFFILIATE:テーマ名] を配置する。記事全体で1〜2回まで。
  同じテーマを2回以上置かない。同じCTAを連続・隣接して置かない（同一広告の重複は禁止）。
- URLやリンクボタンを自分で書かない（必ず [AFFILIATE:テーマ名] のみ）。
  「当サイトは〜のアフィリエイトパートナーです」等の開示文も書かない。
...
【制度・改正の確実性ルール（最重要・本文とFAQで必ず統一）】
- 制度・税制の数値や内容は「①施行済み（確定）」「②改正案・審議中」「③予定・見込み」を必ず区別する。
- 未施行・未確定の内容を、施行済みのように断定しない。必ず「〜される予定」「〜の方向で議論されている」「〜となる見込み」と書く。
- 具体的な上限額・施行時期・適用条件が未確定なら、断定数値を避けるか「（予定）」「（審議中）」と明記し「最終的な内容は公式情報で確認を」と添える。
- 本文・FAQ・会話吹き出しで、同じ事項のトーン（確定/予定/審議中）を絶対に食い違わせない。
  例: 本文で「審議中」と書いたら、FAQでも「〜される予定です（審議中）」とし「引き上げられます」と言い切らない。
...
【デメリット→解消の接続（クリック率向上・必須）】
- 記事内でデメリットや不安（手数料・難しさ・引き出し制限など）に触れた場合、導線文の中で
  「そのデメリットを紹介サービスがどう解消・軽減するか」を必ず1文で具体的に結びつける。
  例: 「口座管理手数料が気になるところですが、私が選んだ松井証券なら運営管理手数料がずっと無料なので、その負担を最小限にできます」
- 一般論ではなく、記事で挙げた具体的なデメリットに対応させること。
...
【筆者独自考察 [INSIGHT]（オリジナリティ・差別化）】
骨格に [INSIGHT] がある場合、そのタグで囲んで筆者ならではの考察を2〜3文書く（[INSIGHT]考察本文[/INSIGHT] の形）。
筆者プロフィール: ${AUTHOR_PROFILE}
- 一般的な制度説明の要約ではなく、「誰にどう効くか」の実践的・独自の切り口にする。
  例:「実際に見てきた経験から言うと、この改正は転職を控えた30代に特に効きます。なぜなら〜」
- プロフィールの強み（投資の実体験・実務経験）を自然に反映する。一人称で書く。
- 「コンサル」「コンサルタント」という単語は本文に書かない。肩書きを名乗らず、語り口で経験を滲ませる。
- 数値の捏造や断定的投資助言にはしない（確実性ルールは [INSIGHT] 内でも守る）。
...
データが本文から読み取れない場合は空配列 [] を返してください。JSON のみ返してください。`;
  ...
  if (bodyMd.includes("[FAQ]")) {
    const faqPrompt = `以下の記事本文を読み、その内容に沿ったよくある質問（FAQ）を5件、JSON配列で生成してください。
...
【整合性ルール（最重要）】
- 回答は必ず上の本文の主張・トーンと一致させる。本文と矛盾する断定をしない。
- 制度の数値・内容が本文で「未確定」「審議中」「予定」と書かれている場合、FAQでも
  「〜される予定です」「現在審議中です」と書き、「〜になります」と断定しない。
- 本文に無い具体的な数値・施行時期を新たに創作しない。不明な点は「公式情報で確認を」と促す。
