...
【アフィリエイト導線ルール（必ず守ること）】
[AFFILIATE:テーマ名] プレースホルダを配置する直前に、以下の形で2〜3文の自然な導線文を書くこと:
1. 自分が実際に使っている/調べた体験に基づく一言（「私自身もこれを使っていて〜」）
2. こんな人におすすめ、という理由（「特に〜な方には向いていると思います」）
3. 行動を促す一文（「まずは口座開設だけでも〜」「公式で内容を確認してみてください」）
押し売り・強引なセールスにならず、悩みの解決策として自然に提示すること。
